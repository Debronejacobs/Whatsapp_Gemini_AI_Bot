...
def get_user_conversation(phone_number):
    ...
    if phone_number not in user_conversations:
        ...
        # Initialize the bot with custom identity for this user
        ...
        user_conversations[phone_number] = convo
    
    return user_conversations[phone_number]
...
def webhook():
    if request.method == "GET":
        ...
        try:
            ...
            sender_phone = request.get_json()["entry"][0]["changes"][0]["value"]["contacts"][0]["wa_id"]
            
            # Get or create conversation for this user
            convo = get_user_conversation(sender_phone)
            
            if data["type"] == "text":
                prompt = data["text"]["body"]
                convo.send_message(prompt)
                send(convo.last.text, sender_phone)
            else:
                ...
                if data["type"] == "audio":
                    ...
                    send("This format is not Supported by the bot ☹", sender_phone)
                    return jsonify({"status": "ok"}), 200
...
def list_conversations():
    """Optional endpoint to view active conversations"""
    active_users = list(user_conversations.keys())
    return jsonify({"active_users": active_users, "count": len(active_users)})

@app.route("/clear/<phone>", methods=["POST"])
def clear_conversation(phone):
    """Optional endpoint to clear a specific user's conversation"""
    if phone in user_conversations:
        del user_conversations[phone]
        return jsonify({"status": "success", "message": f"Conversation for {phone} cleared"})
